...
4. For non-DEC- nodes, set status: draft (BA promotes to active).
   For DEC- nodes: read role from claude.md. If role: BA → set status: approved and add auto_approved_by: BA_role to frontmatter. Otherwise set status: draft as normal.   
...
```
[ABSORB] Completed <frs-id>. Nodes created: <count> (<NODE-IDs>). CNFs raised: <count>. Auto-approved DEC- nodes: <ids or 0>. index.md updated.
```
